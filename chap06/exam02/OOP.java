...
class StudentScore { // 테이블 = 표 = 참조자료형 // 데이터를 효율적으로 관리하기 위한 방법 // 객체지향 
	...
	StudentScore(){ // Constructor (생성자), new 메모리 할당을 요청한 곳에 나옴, 참조자료형의 이름과 같아야한다
...
public class OOP { 	
	public static void main(String[] args) { // 자바는 메인만 관심있다!
		...
		hongkildong = new StudentScore(); // 테이블을 실제 데이터로 저장할때는 new로 지정해준다 // 데이터 지정
		                                  // 주소값을 저장할 시작점 // 4바이트
		...
		KangKamChan = new StudentScore(); // Instance화 > Instance(인스턴스)
